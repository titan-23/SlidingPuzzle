...
namespace puzzle15 {
  ...
  static inline Result run(Param &param, const bool verbose=false) {
    ...
    int state = pool.gen();
    pool.get(state)->init();
    vector<int> keep = { state };

    // seen[pool.get(state)->hash] = 0;
    ...
    for (int turn = 0; turn < param.get_max_turn(); ++turn) {
      ...
      for (const int now_state: keep) {
        const vector<Action> &actions = pool.get(now_state)->get_actions();
      ...
      int best_state = *min_element(keep.begin(), keep.end(), [&] (const int &l, const int &r) {
      ...
      if (pool.get(best_state)->is_done()) {
        cerr << "done" << endl;
        result.history = pool.get(best_state)->history;
